"""
PoseCorrection – per-image or per-frame camera pose refinement.
"""
...
class PoseCorrection(nn.Module):
    def __init__(self, metadata):
        ...
        self.pose_correction_trans = torch.nn.Parameter(
            torch.zeros(num_poses, 3).float().cuda()
        ).requires_grad_(True)
        self.pose_correction_rots = torch.nn.Parameter(
            torch.tensor([[1, 0, 0, 0]]).repeat(num_poses, 1).float().cuda()
        ).requires_grad_(True)
