"""
GaussianModel – base Gaussian point cloud model.

Exact copy from street_gaussians except:
- ``densify_and_prune`` signature uses ``prune_big_points`` bool instead of
  ``max_screen_size``.
"""
...
class GaussianModel(nn.Module):
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        semantics = torch.zeros((fused_point_cloud.shape[0], self.num_classes), dtype=torch.float, device="cuda")
    ...
    def densify_and_prune(self, max_grad, min_opacity, prune_big_points):
        ...
        # Clone and Split        
        self.densify_and_clone(grads, max_grad, self.scene_radius if hasattr(self, 'scene_radius') else self.spatial_lr_scale)
        self.densify_and_split(grads, max_grad, self.scene_radius if hasattr(self, 'scene_radius') else self.spatial_lr_scale)

        # Prune 
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if prune_big_points:
            big_points_ws = self.get_scaling.max(dim=1).values > (self.scene_radius if hasattr(self, 'scene_radius') else self.spatial_lr_scale) * self.percent_big_ws
