"""
Config system – adapted from street_gaussians for KITTI-360 / Windows.

Key changes from the original:
  • ``os.environ['PWD']``  →  ``os.getcwd()``  (Windows compatible)
  • Added ``cfg.optim.semantic_lr`` default
  • CLI uses ``--cfg_file`` (matching our convention)
"""
...
parser.add_argument("--cfg_file", "--config", default="configs/kitti360_drive_0000.yaml", type=str)
