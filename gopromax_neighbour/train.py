...
def save_log_images(log_dir: str, epoch: int, gt, rendered, depth, acc):
    ...
    if depth is not None:
        ...
        d = torch.zeros_like(gt_np)

    if acc is not None:
        a = acc.detach().cpu().float().expand(3, -1, -1)
    else:
        a = torch.zeros_like(gt_np)

    grid = torchvision.utils.make_grid(
        [gt_np, rn_np, d, a], nrow=4, padding=2, normalize=False)
    ...
    for cam in test_cameras:
        ...
        if img_dir is not None:
            ...
            torchvision.utils.save_image(
                image, os.path.join(img_dir, f"{name}_rgb.png"))
            ...
            rgb_np = image.detach().cpu().permute(1, 2, 0).numpy()
            gt_np = gt[:3].detach().cpu().permute(1, 2, 0).numpy()
            diff = ((rgb_np - gt_np) ** 2).sum(axis=-1)
