...
-----
    ...
    python visualize_metrics.py --model_path output/gopromax_neighbour/sky_mask_v1
    python visualize_metrics.py --model_path output/gopromax_neighbour/sky_mask_v1 --save_dir plots
...
---------------
  ...
  6. ``summary.png``           – combined dashboard of all key curves
        ...
        for row in reader:
            ...
            for k, v in row.items():
                try:
                    ...
                    pass  # skip empty optional columns (e.g. lpips)
...
def plot_summary_dashboard(train: dict, evals: dict, save_path: str | None = None):
    ...
    _style()
    epochs_t = train.get("epoch", np.array([]))
...
def find_latest_model_path() -> str | None:
    ...
    # Fallback: look for eval_metrics.csv
    candidates = sorted(root.rglob("eval_metrics.csv"), key=os.path.getmtime)
    if candidates:
        return str(candidates[-1].parent)
    return None
