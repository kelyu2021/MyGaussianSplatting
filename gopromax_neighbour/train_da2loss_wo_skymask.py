"""
GoPro Max Neighbour – Gaussian Splatting Training with DA2 Depth Loss
=====================================================================

Self-contained training script: reads COLMAP data, initialises Gaussians
from a dense point cloud, and trains with monocular depth supervision.
...
Data layout (relative to gopromax_neighbour/)::
    ...
    data/cubemap_faces_mass13k/  <- person/moving-object masks (255=moving, 0=static)
...
def depth_mono_loss(depth_premul, mono_depth, acc, mask=None,
    ...
    depth_premul   : (1, H, W) premultiplied (alpha-weighted) depth from
                                ...
                                disparity.
    mono_depth     : (H, W)    monocular *disparity* from DA-v2 (invalid/unobserved==0)
...
# ═══════════════════════════════════════════════════════════════════════════
#  §8  Dataset Reader  (COLMAP + cubemap images + moving masks)
# ═══════════════════════════════════════════════════════════════════════════
...
def read_scene(
    ...
    depth_dir     : path to monocular depth maps (*_depth_raw.npy, invalid=0)
...
def save_log_images(log_dir: str, epoch: int, gt, rendered, depth_premul, acc,
                    gt_depth=None, moving_mask=None, sky_mask=None):
    """Save a visualisation grid.

    Row 1: GT RGB  | GT depth
    Row 2: render  | pred depth
    """
    import torchvision
    gt_np = gt.detach().cpu()
    ...
    def _depth_to_3ch(t, v_min=None, v_max=None, valid_mask=None,
        ...
        non-finite or non-positive (e.g. unobserved == 0), or marked invalid by
        ...
        pixels (e.g. stray near-Gaussian leaks).
        """
        if t is None:
    ...
    # ``acc`` are masked out: their expected depth is dominated by a single
    # tiny contribution and would otherwise produce spurious bright/dark
    # spikes after disparity inversion + min/max normalisation.
    pred_acc_t = None
    ...
    # brightness = value, unobserved/zero rendered black.
    #
    ...
    if pred_d_t is not None:
        ...
        if sky_mask is not None:
            # Exclude sky pixels (sky_mask > 0.5 = non-sky/valid)
            sm = sky_mask.detach().cpu().float() if hasattr(sky_mask, 'detach') \
                else torch.as_tensor(sky_mask).float()
            sm = sm.squeeze()
            if sm.ndim == 2:
                sm = sm.unsqueeze(0)
            pred_valid = pred_valid & (sm > 0.5)
    ...
    # GT moving-mask panel (cubemap_faces_sam_moving): white=static, black=moving object
    if moving_mask is not None:
        mm = moving_mask.detach().cpu().float() if hasattr(moving_mask, 'detach') \
            else torch.as_tensor(moving_mask).float()
        mm = mm.squeeze()
        if mm.ndim == 2:
            mm = mm.unsqueeze(0)
        gt_opacity_panel = (1.0 - mm.clamp(0.0, 1.0)).expand(3, -1, -1).contiguous()
    else:
        gt_opacity_panel = torch.ones(3, h, w)

    # Predicted opacity panel (accumulated alpha from rasteriser)
    if acc is not None:
        acc_t = acc.detach().cpu().float() if hasattr(acc, 'detach') \
            else torch.as_tensor(acc).float()
        acc_t = acc_t.squeeze()
        if acc_t.ndim == 2:
            acc_t = acc_t.unsqueeze(0)
        pred_opacity_panel = acc_t.expand(3, -1, -1).contiguous()
    else:
        pred_opacity_panel = torch.zeros(3, h, w)

    # 2×3 grid: GT RGB | GT depth | GT opacity
    #           render | pred depth | pred opacity
    panels = [gt_np, gd, gt_opacity_panel, rn_np, d, pred_opacity_panel]
...
def evaluate(
    ...
    for cam in test_cameras:
        ...
        # ── Save grid: rows=[GT, render, depth/alpha] ───────────
        if img_dir is not None:
            ...
            gt_mask = cam.guidance.get("mask")  # used for depth erosion below
            ...
            # Stricter acc cutoff: stray near-Gaussians often
            # leak with acc < 0.05 and cause bright outliers after disparity
            # inversion.
            pred_valid = acc_np > 5e-2
            # If a GT mask is available, erode it by a few pixels
            ...
            # Both depth panels use linear min/max normalisation,
            # brightness = disparity (= near), unobserved/zero rendered
            ...
            if md is not None:
                ...
                if gt_mask is not None:
                    gm_sky = gt_mask.detach().float().cpu().numpy().squeeze()
                    if gm_sky.shape == md_valid.shape:
                        md_valid = md_valid & (gm_sky > 0.5)
            ...
            # ── GT moving-mask panel (cubemap_faces_sam_moving) ──────────────
            # white=static, black=moving object (1 - mask)
            moving_mask_vis = cam.guidance.get("moving_mask")
            if moving_mask_vis is not None:
                mm_f = moving_mask_vis.detach().float().cpu().numpy().squeeze()
                gt_opacity_uint8 = np.clip((1.0 - mm_f) * 255.0, 0, 255).astype(np.uint8)
            else:
                gt_opacity_uint8 = np.full((H, W), 255, dtype=np.uint8)
            gt_opacity_img = np.stack([gt_opacity_uint8] * 3, axis=-1)

            # ── Predicted opacity panel (accumulated alpha) ───────────────────
            pred_opacity_uint8 = np.clip(acc_np * 255.0, 0, 255).astype(np.uint8)
            pred_opacity_img = np.stack([pred_opacity_uint8] * 3, axis=-1)

            # ── Compose grid ─────────────────────────────────────────
            # 2×3: GT RGB | GT depth | GT opacity
            #      render | pred depth | pred opacity
            row1 = np.concatenate([gt_rgb_img,           gt_depth_img,   gt_opacity_img],   axis=1)
            row2 = np.concatenate([_to_rgb_uint8(image), pred_depth_img, pred_opacity_img], axis=1)
...
def training(cfg: dict):
    ...
    for epoch in range(start_epoch + 1, num_epochs + 1):
        ...
        for cam_idx, cam in enumerate(viewpoint_stack):
            ...
            # Exclude moving objects from all losses
            ...
            # ── Accumulation mask supervision (BCE) ──────────────────
            lambda_am = optim_cfg.get("lambda_acc_mask", 0.0)
            if lambda_am > 0 and "acc_mask" in cam.guidance:
                gt_acc_mask = cam.guidance["acc_mask"]
                gt_acc_mask = (gt_acc_mask.cuda(non_blocking=True)
                               if not gt_acc_mask.is_cuda else gt_acc_mask)
                gt_acc_mask = gt_acc_mask.float()  # [1, H, W]  0/1
                a = acc.clamp(1e-6, 1.0 - 1e-6)
                am_loss = -(gt_acc_mask * a.log() +
                            (1.0 - gt_acc_mask) * (1.0 - a).log())
                if mask is not None:  # exclude moving objects
                    am_loss = am_loss * mask.float()
                    am_loss = am_loss.sum() / mask.float().sum().clamp(min=1)
                else:
                    am_loss = am_loss.mean()
                am_loss = lambda_am * am_loss
                scalar_dict["acc_mask_loss"] = am_loss.item()
                loss += am_loss
