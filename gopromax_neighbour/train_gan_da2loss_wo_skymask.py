...
def save_log_images_gan(
    ...
    moving_mask=None, mono_depth=None,
):
    """Save a 3×3 grid to {log_dir}/epoch_{epoch:04d}.png.

    Row 1: GT RGB          | GT mask (moving=green)          | GT depth (mono)
    Row 2: On-road RGB     | |on-road − GT| error map        | On-road depth
    Row 3: Jittered RGB    | |jitter − on-road| diff map     | Jittered depth

    All depth panels: greyscale, light = near, dark = far, empty = black.
    Error/diff panels: amplified absolute difference, light = large error.
    """
    import numpy as np
    ...
    def _depth_to_grey(depth_t_or_np, valid_mask=None, is_disparity=False,
        ...
        valid pixels, suppressing the influence of a few outliers.
        """
    ...
    def _pred_depth_panel(depth_premul_t, acc_t, acc_cutoff: float = 5e-2):
    ...
    # Error map: |on-road - GT|, amplified ×4 for visibility
    err_on = np.abs(
        rendered_on.detach().cpu().float().numpy().transpose(1, 2, 0)[:, :, :3] -
        gt_on.detach().cpu().float().numpy().transpose(1, 2, 0)[:, :, :3]
    )
    err_on_img = np.clip(err_on * 4.0 * 255.0, 0, 255).astype(np.uint8)
    pred_depth_img = _pred_depth_panel(depth_on, acc_on)

    # ── Row 3: Jittered prediction ────────────────────────────────────
    jit_rgb = _to_rgb_uint8(rendered_off)
    # Diff map: |jitter - on-road|, amplified ×4 for visibility
    diff_off = np.abs(
        rendered_off.detach().cpu().float().numpy().transpose(1, 2, 0)[:, :, :3] -
        rendered_on.detach().cpu().float().numpy().transpose(1, 2, 0)[:, :, :3]
    )
    diff_off_img = np.clip(diff_off * 4.0 * 255.0, 0, 255).astype(np.uint8)
    jit_depth_img = _pred_depth_panel(depth_off, acc_off, acc_cutoff=1e-1)

    # ── Opacity panels (accumulated alpha from rasteriser) ────────────
    def _acc_to_grey(acc_t):
        """Convert acc tensor to greyscale RGB: white = fully opaque."""
        a = acc_t.detach().cpu().numpy().squeeze().astype(np.float32)
        out = np.clip(a * 255.0, 0, 255).astype(np.uint8)
        return np.stack([out] * 3, axis=-1)

    opacity_on_img  = _acc_to_grey(acc_on)
    opacity_off_img = _acc_to_grey(acc_off)

    # GT opacity: white=static (not masked), black=masked-out moving object
    if moving_mask is not None:
        mm = moving_mask.detach().cpu().float().numpy().squeeze()
        gt_opacity_uint8 = np.clip((1.0 - mm.clip(0, 1)) * 255.0, 0, 255).astype(np.uint8)
        gt_opacity_img = np.stack([gt_opacity_uint8] * 3, axis=-1)
    else:
        gt_opacity_img = np.full((H, W, 3), 255, dtype=np.uint8)

    # ── Compose grid (3×4) ────────────────────────────────────────────
    # Col: RGB | error/diff | depth | opacity
    row1 = np.concatenate([gt_rgb,   mask_img,     gt_depth_img,   gt_opacity_img],  axis=1)
    row2 = np.concatenate([pred_rgb, err_on_img,   pred_depth_img, opacity_on_img],  axis=1)
    row3 = np.concatenate([jit_rgb,  diff_off_img, jit_depth_img,  opacity_off_img], axis=1)
...
def _save_eval_jitter_videos(
    ...
    eval image can reuse it instead of rendering that endpoint again.
    """
    ...
    # image_on is used as the base frame for jitter videos
...
# ═══════════════════════════════════════════════════════════════════════════
#  §0b  GAN-aware Evaluation (3×3 grid per test camera)
# ═══════════════════════════════════════════════════════════════════════════
...
def evaluate_gan(
    ...
    """Evaluate on test cameras with 3×3 grid images.

    Only cameras whose face is in ``jitter_faces`` are evaluated (when
    given), and one grid image is saved per (camera, jitter direction)
    ...
    for cam in eval_cameras:
        ...
        image_on_display = pkg_on["rgb"].clamp(0.0, 1.0)

        # Metrics on the rendered image
        ...
        # Render off-road for each requested direction and save grid image
        for direction in directions:
            jit_cam = build_jittered_camera(
            ...
            image_off_display = pkg_off["rgb"].clamp(0.0, 1.0)
            if img_dir is not None:
                name = cam.image_name
...
# ═══════════════════════════════════════════════════════════════════════════
#  §3b  Training Monitor Plots
# ═══════════════════════════════════════════════════════════════════════════

def generate_monitor_plots(
    train_csv_path: str,
    eval_csv_path: str | None,
    out_dir: str,
) -> None:
    """Read training/eval CSV logs and save PNG monitor plots to *out_dir*.

    Panels (training_monitor.png):
      Row 0: losses | Wasserstein distance | PSNR on-road
      Row 1: critic scores ±std | gradient penalty | critic grad norm
      Row 2: critic weight norm | aux losses (acc/depth) | # Gaussians

    A separate eval_monitor.png is written when eval CSV data is present.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        print(f"[monitor] Skipping plots – missing dependency ({exc})")
        return

    os.makedirs(out_dir, exist_ok=True)

    if not os.path.isfile(train_csv_path):
        print(f"[monitor] Train CSV not found: {train_csv_path}")
        return
    try:
        df = pd.read_csv(train_csv_path)
    except Exception as exc:
        print(f"[monitor] Failed to read train CSV: {exc}")
        return
    if df.empty:
        return
    ...
    fig.suptitle("GAN Training Monitor", fontsize=14, fontweight="bold")
    ...
    # [0,0] Losses
    ax = axes[0, 0]
    ax.plot(epochs, df["loss_critic"], label="critic",  color="tab:red")
    ax.plot(epochs, df["loss_gen"],    label="gen",      color="tab:blue")
    ax.plot(epochs, df["loss_recon"],  label="recon",    color="tab:green")
    ax.set_title("Losses"); ax.set_xlabel("epoch"); ax.legend(); ax.grid(True)

    # [0,1] Wasserstein distance
    ax = axes[0, 1]
    ax.plot(epochs, df["wasserstein_dist"], color="tab:purple")
    ax.axhline(0.0,  color="k",      ls="--", lw=0.8, label="0")
    ax.axhline(20.0, color="tab:red", ls=":",  lw=0.8, label="overfit (20)")
    ax.axhline(0.01, color="tab:orange", ls=":", lw=0.8, label="underfit (0.01)")
    ax.set_title("Wasserstein Distance"); ax.set_xlabel("epoch")
    ax.legend(fontsize=8); ax.grid(True)

    # [0,2] PSNR on-road
    ax = axes[0, 2]
    ax.plot(epochs, df["psnr_onroad"], color="tab:green")
    ax.set_title("PSNR On-road"); ax.set_xlabel("epoch"); ax.grid(True)

    # ── Row 1 ─────────────────────────────────────────────────────────────
    # [1,0] Critic scores ±std
    ax = axes[1, 0]
    rm = _col("real_score_mean"); fm = _col("fake_score_mean")
    rs = _col("real_score_std");  fs = _col("fake_score_std")
    if rm is not None:
        ax.plot(epochs, rm, label="real", color="tab:blue")
        ax.plot(epochs, fm, label="fake", color="tab:orange")
        if rs is not None:
            ax.fill_between(epochs, rm - rs, rm + rs, alpha=0.2, color="tab:blue")
        if fs is not None:
            ax.fill_between(epochs, fm - fs, fm + fs, alpha=0.2, color="tab:orange")
        ax.axhline(0.0, color="k", ls="--", lw=0.7)
    ax.set_title("Critic Scores (mean \u00b1 std)")
    ax.set_xlabel("epoch"); ax.legend(fontsize=8); ax.grid(True)

    # [1,1] Gradient penalty
    ax = axes[1, 1]
    gp_vals = _col("gp")
    if gp_vals is not None:
        ax.plot(epochs, gp_vals, color="tab:brown")
        ax.axhline(5.0, color="tab:red", ls=":", lw=0.8, label="GP_HIGH (5)")
        ax.legend(fontsize=8)
    ax.set_title("Gradient Penalty (GP)"); ax.set_xlabel("epoch"); ax.grid(True)

    # [1,2] Critic grad norm
    ax = axes[1, 2]
    gn_vals = _col("critic_grad_norm")
    if gn_vals is not None:
        ax.plot(epochs, gn_vals, color="tab:red")
        ax.axhline(100.0, color="tab:red", ls=":", lw=0.8, label="explode (100)")
        ax.legend(fontsize=8)
    ax.set_title("Critic Grad Norm"); ax.set_xlabel("epoch"); ax.grid(True)

    # ── Row 2 ─────────────────────────────────────────────────────────────
    # [2,0] Critic weight norm
    ax = axes[2, 0]
    wn_vals = _col("critic_weight_norm")
    if wn_vals is not None:
        ax.plot(epochs, wn_vals, color="tab:gray")
    ax.set_title("Critic Weight Norm"); ax.set_xlabel("epoch"); ax.grid(True)

    # [2,1] Aux losses
    ax = axes[2, 1]
    ax.plot(epochs, df["acc_loss"],   label="acc",   color="tab:cyan")
    ax.plot(epochs, df["depth_loss"], label="depth", color="tab:olive")
    ax.set_title("Aux Losses"); ax.set_xlabel("epoch")
    ax.legend(fontsize=8); ax.grid(True)

    # [2,2] # Gaussians
    ax = axes[2, 2]
    ax.plot(epochs, df["n_points"], color="tab:purple")
    ax.set_title("# Gaussians"); ax.set_xlabel("epoch"); ax.grid(True)

    plt.tight_layout()
    out_train = os.path.join(out_dir, "training_monitor.png")
    fig.savefig(out_train, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"[monitor] Saved training plot  -> {out_train}")

    # ── Eval CSV ──────────────────────────────────────────────────────────
    if eval_csv_path and os.path.isfile(eval_csv_path):
        try:
            edf = pd.read_csv(eval_csv_path)
        except Exception:
            return
        if edf.empty:
            return
        fig2, axes2 = plt.subplots(1, 3, figsize=(15, 4))
        fig2.suptitle("GAN Eval Monitor (test split)",
                      fontsize=13, fontweight="bold")
        for col, ax, color, title in [
            ("psnr",    axes2[0], "tab:green", "PSNR"),
            ("ssim",    axes2[1], "tab:blue",  "SSIM"),
            ("l1_loss", axes2[2], "tab:red",   "L1 Loss"),
        ]:
            if col in edf.columns:
                ax.plot(edf["epoch"], edf[col],
                        color=color, marker="o", ms=4)
            ax.set_title(title); ax.set_xlabel("epoch"); ax.grid(True)
        plt.tight_layout()
        out_eval = os.path.join(out_dir, "eval_monitor.png")
        fig2.savefig(out_eval, dpi=120, bbox_inches="tight")
        plt.close(fig2)
        print(f"[monitor] Saved eval plot     -> {out_eval}")
...
def training_gan(cfg: dict, model_root: str, road_width: float,
    ...
    # ── Create Critic ─────────────────────────────────────────────────
    critic = Critic(in_channels=3, base_channels=32).cuda()
    ...
    if not os.path.isfile(train_csv_path):
        with open(train_csv_path, "w", newline="") as f:
            ...
            writer.writerow([
                ...
                "wasserstein_dist", "acc_loss", "depth_loss", "psnr_onroad",
                "n_points",
                "real_score_mean", "real_score_std",
                "fake_score_mean", "fake_score_std",
                "gp", "critic_grad_norm", "critic_weight_norm",
            ])
        ...
        for wi, cam in enumerate(wu_stack):
            for direction in _wu_directions:
                ...
                with torch.no_grad():
                    ...
                    real_img = real_pkg["rgb"].clamp(0.0, 1.0).detach()
                    fake_img = fake_pkg["rgb"].clamp(0.0, 1.0).detach()
    ...
    for epoch in range(1, gan_epochs + 1):
        ...
        ep_depth_loss = 0.0
        ep_count = 0
        # Critic health accumulators
        ep_real_score_sum = 0.0
        ep_real_score_sq  = 0.0
        ep_fake_score_sum = 0.0
        ep_fake_score_sq  = 0.0
        ep_gp_sum          = 0.0
        ep_critic_grad_sum = 0.0
        ep_critic_count    = 0
        ...
        log_moving_mask = log_mono_depth = None
        ...
        for cam_idx, cam in enumerate(viewpoint_stack):
            ...
            mask = (~moving_mask) if moving_mask is not None else None
            recon_mask = mask
            ...
            for dir_idx, direction in enumerate(_jitter_directions):
                ...
                if global_step_idx % (critic_iters + 1) < critic_iters:
                    ...
                    critic_optimizer.zero_grad()
                    loss_c.backward()
                    # Compute critic gradient norm (before the weight update)
                    _gnorms = [p.grad.detach().norm() ** 2
                               for p in critic.parameters() if p.grad is not None]
                    _critic_grad_norm = (torch.stack(_gnorms).sum().sqrt().item()
                                        if _gnorms else 0.0)
                    critic_optimizer.step()

                    ep_critic_loss += loss_c.item()
                    ep_w_dist      += w_dist.item()
                    # Accumulate health stats
                    _rs = real_score.mean().item()
                    _fs = fake_score.mean().item()
                    ep_real_score_sum += _rs
                    ep_real_score_sq  += _rs ** 2
                    ep_fake_score_sum += _fs
                    ep_fake_score_sq  += _fs ** 2
                    ep_gp_sum          += gp.item()
                    ep_critic_grad_sum += _critic_grad_norm
                    ep_critic_count    += 1
                ...
                # ==================================================
                    ...
                    Ll1 = l1_loss(real_img, gt_image, recon_mask)
                    loss_recon = (
                        (1.0 - lambda_dssim) * lambda_l1 * Ll1 +
                        lambda_dssim * (1.0 - ssim(real_img, gt_image,
                                                    mask=recon_mask)))
                    ...
                    # --- Alpha/acc (mean accumulation) ---
                    acc_loss = real_pkg["acc"].mean()
                    ...
                    ep_acc_loss += acc_loss.item()
                    ep_depth_loss += depth_loss.item()

                    with torch.no_grad():
                        cur_psnr_on = psnr(
                            real_img.clamp(0.0, 1.0),
                    ...
                    if dir_idx == len(_jitter_directions) - 1:
                        ...
                        log_render_on = real_img.detach().clamp(0.0, 1.0)
                        log_render_off = fake_img.detach().clamp(0.0, 1.0)
        ...
        avg_acc = ep_acc_loss / n_gen_steps
        avg_depth = ep_depth_loss / n_gen_steps
        ...
        with torch.no_grad():
            ...
            if ep_critic_count > 0:
                _avg_real = ep_real_score_sum / ep_critic_count
                _avg_fake = ep_fake_score_sum / ep_critic_count
                _std_real = math.sqrt(max(
                    ep_real_score_sq / ep_critic_count - _avg_real ** 2, 0.0))
                _std_fake = math.sqrt(max(
                    ep_fake_score_sq / ep_critic_count - _avg_fake ** 2, 0.0))
                _avg_gp        = ep_gp_sum / ep_critic_count
                _avg_cgrad     = ep_critic_grad_sum / ep_critic_count
                _wnorms = [p.detach().norm() ** 2 for p in critic.parameters()]
                _critic_wnorm  = torch.stack(_wnorms).sum().sqrt().item()
            else:
                _avg_real = _avg_fake = _std_real = _std_fake = 0.0
                _avg_gp = _avg_cgrad = _critic_wnorm = 0.0

            # Health flags
            # W_dist < 0.01  → critic can't separate real/fake (underfit/collapse)
            # W_dist > 20    → critic too dominant (overfit)
            # score std < 0.01 for both → critic outputs near-constant (collapse)
            # avg_gp > 5     → interpolated gradients far from unit norm (instability)
            # avg_cgrad > 100 → critic gradient explosion
            _flags = []
            if avg_w < 0.01:
                _flags.append("UNDERFIT/COLLAPSE")
            if avg_w > 20.0:
                _flags.append("OVERFIT")
            if ep_critic_count > 0 and _std_real < 0.01 and _std_fake < 0.01:
                _flags.append("SCORE_COLLAPSE")
            if _avg_gp > 5.0:
                _flags.append("GP_HIGH")
            if _avg_cgrad > 100.0:
                _flags.append("GRAD_EXPLODE")
            _health = "OK" if not _flags else " | ".join(_flags)
            ...
            tqdm.write(
                ...
                f"  Critic│ real={_avg_real:+.3f}±{_std_real:.3f}  "
                f"fake={_avg_fake:+.3f}±{_std_fake:.3f}  "
                f"GP={_avg_gp:.4f}  "
                f"grad_norm={_avg_cgrad:.2f}  "
                f"weight_norm={_critic_wnorm:.2f}  "
                f"[{_health}]\n"
                f"  PSNR  │ on-road={avg_psnr_on:.2f}  "
                f"acc={avg_acc:.4f}  depth={avg_depth:.6f}\n"
        ...
        with open(train_csv_path, "a", newline="") as f:
            ...
            writer.writerow([
                ...
                avg_acc, avg_depth, avg_psnr_on, gaussians.num_points,
                _avg_real, _std_real, _avg_fake, _std_fake,
                _avg_gp, _avg_cgrad, _critic_wnorm,
            ])
        ...
        if tb_writer:
            ...
            tb_writer.add_scalar("gan/acc_loss",         avg_acc, epoch)
            tb_writer.add_scalar("gan/depth_loss",       avg_depth, epoch)
            tb_writer.add_scalar("gan/psnr_onroad",      avg_psnr_on, epoch)
            # Critic health
            tb_writer.add_scalar("critic/real_score_mean",  _avg_real,      epoch)
            tb_writer.add_scalar("critic/fake_score_mean",  _avg_fake,      epoch)
            tb_writer.add_scalar("critic/real_score_std",   _std_real,      epoch)
            tb_writer.add_scalar("critic/fake_score_std",   _std_fake,      epoch)
            tb_writer.add_scalar("critic/gp",               _avg_gp,        epoch)
            tb_writer.add_scalar("critic/grad_norm",         _avg_cgrad,     epoch)
            tb_writer.add_scalar("critic/weight_norm",       _critic_wnorm,  epoch)
    ...
    generate_monitor_plots(
        train_csv_path=train_csv_path,
        eval_csv_path=dirs.get("eval_csv_path"),
        out_dir=dirs["model_path"],
    )
...
def main():
    ...
    parser.add_argument(
        "--critic_iters", type=int, default=2,
        help="Critic updates per generator update (default: 2).")
    parser.add_argument(
    ...
    parser.add_argument(
        "--lambda_adv", type=float, default=5.0,
        help="Adversarial loss weight on the generator step (default: 5.0).")
    parser.add_argument(
