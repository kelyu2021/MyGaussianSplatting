"""
General utilities – adapted from street_gaussians.

Key change: ``roma`` import is lazy (inside ``quaternion_slerp``) so the
module can be imported even when ``roma`` is not installed.  ``roma`` is
only needed for actor-tracking interpolation which KITTI-360 does not use.
"""
...
def NumpytoTorch(image, resolution, resize_mode=cv2.INTER_AREA):
    ...
    if len(image.shape) == 2:
        image = image[..., None].permute(2, 0, 1)
...
def quaternion_slerp(q0: torch.Tensor, q1: torch.Tensor, step=0.5) -> torch.Tensor:
    """Slerp two quaternions.  Requires ``roma`` (lazy import)."""
    import roma  # lazy – only needed for actor tracking
