"""
GoPro 360 Gaussian Splatting – Rendering / Evaluation Script
=============================================================
...
def render_sets():
    ...
    with torch.no_grad():
        ...
        if times:
            print(f"\nRendering times (ms): mean={sum(times[1:]) / max(len(times) - 1, 1):.2f}")
...
def _save_panoramic(rgbs, rgbs_gt, depths, diffs, cams_list,
    ...
    The original 360° equirectangular image reads left-to-right as:
        left → front → right → back (wrapping)
    ...
    def _build_strip(frames_list):
        """Build horizontal strip frames [left|front|right|back]."""
        strip_frames = []
...
def render_trajectory():
    ...
    with torch.no_grad():
        ...
        # ── Panoramic videos (2×2 grid: front/right/back/left) ───
