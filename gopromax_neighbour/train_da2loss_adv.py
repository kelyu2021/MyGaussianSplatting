...
class GaussianModel:
    ...
    def densify_and_prune(
        ...
        percent_big_ws=0.1, force_split_screen_size=0,
    ):
        ...
        # Force-split oversized visible Gaussians regardless of position gradient.
        # Nearby Gaussians project to large screen-space radii but may have weak
        # position gradients (shifting the centre doesn't help a blob that is
        # simply too large). This ensures they are split into finer detail even
        # when the gradient test does not fire.
        if force_split_screen_size > 0:
            oversized = (
                (self.max_radii2D > force_split_screen_size) &
                (self.get_opacity > 0.1).squeeze(-1) &
                (self.get_scaling.max(dim=1).values >
                 self.percent_dense * scene_extent)
            )
            if oversized.any():
                self._densify_and_split_by_mask(oversized)
    ...
    def _densify_and_split_by_mask(self, selected, N=2):
        """Split Gaussians selected by mask, ignoring position gradient.

        Used to force-split oversized Gaussians near the camera that have
        weak position gradients (the centre is well-placed but the splat is
        too large to represent fine detail).
        """
        if not selected.any():
            return
        stds = self.get_scaling[selected].repeat(N, 1)
...
DEFAULT_CFG = {
    ...
    "optim": {
        ...
        "opacity_reset_until_epoch": 60,
        "prune_epoch_interval": 30,
        "prune_min_opacity": 0.003,
        "force_split_screen_size": 15,
        "depth_loss_trim": 0.05,
        "lambda_l1": 1.0,
        "lambda_dssim": 0.2,
        "lambda_sky_acc": 0.01,
        "lambda_sh_reg": 0.001,
    ...
    for epoch in range(start_epoch + 1, num_epochs + 1):
        ...
        for cam_idx, cam in enumerate(viewpoint_stack):
            ...
            if lambda_depth > 0 and "mono_depth" in cam.guidance:
                ...
                # Use a small trim to avoid discarding close-range pixels.
                # The default trim=0.2 discards the 20% largest residuals,
                # which are predominantly high-disparity (close-range) pixels.
                depth_trim = optim_cfg.get("depth_loss_trim", 0.05)
                d_loss = lambda_depth * depth_mono_loss(
                    depth_premul, mono_depth, acc, mask, trim=depth_trim)
                ...
                if epoch <= densify_until_epoch:
                    ...
                    if (epoch > densify_from_epoch and
                        ...
                        n_before, n_after = gaussians.densify_and_prune(
                            ...
                            force_split_screen_size=optim_cfg.get("force_split_screen_size", 0),
                        )
                        if n_before != n_after:
        ...
        with torch.no_grad():
            ...
            # ── Opacity reset ─────────────────────────────────────────
            # Stop resetting opacity after opacity_reset_until_epoch.
            # Late resets force close-range Gaussians (which cover many
            # pixels each) to spend many optimizer steps rebuilding opacity
            # instead of refining shape and colour.
            opacity_reset_until = optim_cfg.get(
                "opacity_reset_until_epoch", densify_until_epoch)
            if (epoch <= densify_until_epoch and
                    epoch <= opacity_reset_until and
