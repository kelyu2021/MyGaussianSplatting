"""
SkyCubeMap – cube-map based sky model using nvdiffrast.

Requires ``nvdiffrast`` (``pip install nvdiffrast``).
"""
...
try:
    ...
    dr = None  # optional – only needed when include_sky is True
...
class SkyCubeMap(nn.Module):
    ...
    def forward(self, camera: Camera, acc=None):
        ...
        if mask is None:
            ...
            sky_color = sky_color[0].permute(2, 0, 1).clamp(0., 1.)
        else:
            if cfg.mode == 'train':
                if self.cfg.white_background:
            ...
            sky_color = sky_color.permute(2, 0, 1).clamp(0., 1.)
