...
def export_to_gs_ply(
    ...
    save_path = os.path.join(export_dir, "scene.ply")
...
def export_to_gsplat(
    prediction: Prediction,
    export_dir: str,
    gs_views_interval: Optional[int] = 1,
):
    """
    Export Gaussians to .splat binary format (antimatter15/splat compatible).
    ...
    src_v, out_h, out_w, _ = pred_depth.shape

    if gs_views_interval is None:
        gs_views_interval = max(pred_depth.shape[0] // 12, 1)

    # Build mask (same logic as gs_ply export)
    mask = torch.zeros_like(pred_depth, dtype=torch.bool)
    gstrim_h = int(8 / 256 * out_h)
    gstrim_w = int(8 / 256 * out_w)
    mask[:, gstrim_h:-gstrim_h, gstrim_w:-gstrim_w, :] = 1

    d_percentile = torch.quantile(
        pred_depth.view(pred_depth.shape[0], -1), q=0.9, dim=1
    ).view(-1, 1, 1)
    d_mask = (pred_depth[..., 0] <= d_percentile).unsqueeze(-1)
    mask = mask & d_mask
    mask = mask.squeeze(-1)
    ...
    def trim_select_reshape(element):
        ...
        return selected[::gs_views_interval][mask[::gs_views_interval]]

    means = trim_select_reshape(gs_world.means).detach().cpu().numpy()
    scales = trim_select_reshape(gs_world.scales).detach().cpu()
    rotations = trim_select_reshape(gs_world.rotations).detach().cpu().numpy()
    harmonics = trim_select_reshape(gs_world.harmonics).detach().cpu()
    opacities = trim_select_reshape(gs_world.opacities).detach().cpu()

    # Convert SH DC band to RGB color [0, 255]
    sh_c0 = 0.2820947917738781  # 1 / (2 * sqrt(pi))
    colors_f = harmonics[..., 0] * sh_c0 + 0.5  # (N, 3)
    colors_f = colors_f.clamp(0.0, 1.0).numpy()
    rgb = (colors_f * 255).astype(np.uint8)

    # Convert opacity to alpha [0, 255] via sigmoid
    alpha = torch.sigmoid(opacities).clamp(0.0, 1.0).numpy()
    alpha = (alpha * 255).astype(np.uint8)

    # Log-space scales
    log_scales = scales.log().numpy()

    # Normalize quaternion to [0, 255] range: map [-1, 1] -> [0, 255]
    rot_norm = rotations / (np.linalg.norm(rotations, axis=-1, keepdims=True) + 1e-8)
    rot_u8 = ((rot_norm * 0.5 + 0.5) * 255).astype(np.uint8)

    n = means.shape[0]
    os.makedirs(export_dir, exist_ok=True)
    save_path = os.path.join(export_dir, "scene.splat")
    ...
    with open(save_path, "wb") as f:
        for i in range(n):
            ...
            # color: RGBA as 4x uint8
            f.write(struct.pack("BBBB", rgb[i, 0], rgb[i, 1], rgb[i, 2], alpha[i]))
            # rotation: quaternion as 4x uint8
            f.write(struct.pack("BBBB", rot_u8[i, 0], rot_u8[i, 1], rot_u8[i, 2], rot_u8[i, 3]))
